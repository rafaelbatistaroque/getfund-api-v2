...
// GreaterThan verifica se o valor é maior que o esperado
func (v *Verifier) GreaterThan(threshold interface{}) *Verifier {
...
// LowerThan verifica se o valor é menor que o esperado
func (v *Verifier) LowerThan(threshold interface{}) *Verifier {
	val, ok := v.value.(float64)
	if !ok {
		if intVal, isInt := v.value.(int); isInt {
			val = float64(intVal)
