...
func New(repository auth_contract.Repository, hasher security.Hasher, cache cache_service.Cache, bus bus.EventBus, settings settings.ApplicationSettings) create_user.UseCase {
...
func (c *createUserApplication) Execute(input *create_user.Input) (*create_user.Output, *result_app.ApplicationError) {
	...
	if validated.IsInvalid() {
		return nil, result_app.New(result_app.UNPROCESSABLE_CONTENT_CODE, validated.GetErrors())
	}
...
func validateDuplicatedUser(input *create_user.Input, repository auth_contract.Repository) *result_app.ApplicationError {
